...
async function showTabStore(tabs){
  ...
  let tab = tabs.pop();
    ...
    const tabStore = await browser.tabs.sendMessage(tab.id, {code: "tab"})
    ...
    if (tabStore.response.length > 0) {
      ...
      let content = document.createTextNode("No Conexions in this tab.");
...
getActiveTab().then(showTabStore);
